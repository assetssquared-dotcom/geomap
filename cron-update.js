// cron-update.js — 매일 한국시간 오전 9시 자동 실행
// Vercel Cron이 UTC 00:00에 호출 = KST 09:00
// 국가 데이터 + 뉴스를 한 번에 재생성해서 KV에 저장

export default async function handler(req, res) {
  // Vercel Cron 인증 확인 (외부에서 무단 호출 방지)
  ...
  async function callHaiku(prompt, system, maxTokens) {
  ...
  const COUNTRY_PROMPT = (ids) =>
    `오늘(${today}) 아래 국가들 최신 데이터 JSON:\n${ids.map(id=>`${id}:${CONTEXT[id]||id}`).join("\n")}\n\n형식: {"국가ID":{"summary":"2문장","keywords":["k1","k2","k3","k4"],"rate":{"name":"중앙은행","val":"금리","trend":"hawk|dove|hold","trendLabel":"레이블","note":"1~2문장"},"policy":[{"text":"<b>정책</b> — 설명"},{"text":"<b>정책</b> — 설명"}],"watchlist":[{"icon":"📌","text":"<b>항목</b> — 설명"},{"icon":"📌","text":"<b>항목</b> — 설명"}],"risk":["r1","r2","r3"]}} JSON만.`;

  try {
    const results = {};
    for (const batch of BATCHES) {
      const data = await callHaiku(COUNTRY_PROMPT(batch), SYS, 2500);
      Object.assign(results, data);
    }

    // 뉴스도 함께 갱신
    const newsItems = await callHaiku(
      `오늘(${today} 오전 세션) 투자자 필수 지정학·경제 뉴스 8개 JSON:\n[{"date":"YYYY.MM.DD","category":"지정학|전쟁|무역|에너지|금리|외교|자원","title":"20자 이내","body":"2~3문장","impact":"투자 시사점 1문장"}]\nJSON만.`,
      SYS, 1200
    );
    ...
    const newsPayload = {
      ...
      fetchedAt: now.toISOString(),
      fetchedAtKr: `${today} 오전 9시`,
      nextUpdateKr: `${today} 오후 9시`,
      source: "cron"
    };

    // KV에 24시간 저장
    await kvSet("geomap:country-data:v3", countryPayload, 86400);
    await kvSet("geomap:news:v1", newsPayload, 86400);

    console.log(`[Cron] ${today} 업데이트 완료`);
