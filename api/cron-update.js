export default async function handler(req, res) {
  ...
  async function callSonnet(prompt, useWebSearch = false, maxTokens = 4000) {
    const body = {
  ...
  try {
    ...
    try {
      ...
      batchErrors.push(`[connections] ${connErr.message}`);
    }
    ...
    if (batchErrors.length) results.batchErrors = batchErrors;
