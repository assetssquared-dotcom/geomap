export default async function handler(req, res) {
  ...
  try {
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model: "claude-sonnet-4-20250514",  // Sonnet — 웹 검색 지원
        max_tokens: 2000,
        tools: [{ type: "web_search_20250305", name: "web_search" }],
        system: `오늘은 ${today}입니다. 반드시 웹 검색으로 오늘의 실제 최신 데이터를 확인하세요. 유가·금리·환율 등 수치는 반드시 검색된 실제 값을 사용하세요. 추측하거나 오래된 데이터를 사용하지 마세요.`,
        messages: [{
          role: "user",
          content: `웹 검색으로 오늘(${today}) 실제 최신 지정학·경제 뉴스를 확인한 후, 투자자 필수 뉴스 8개를 JSON 형식으로만 출력하세요:\n[{"date":"${dateStr}","category":"지정학|전쟁|무역|에너지|금리|외교|자원","title":"15자 이내","body":"2문장 (실제 수치 포함)","impact":"시사점 1문장"}]\nJSON만.`
        }]
      })
    });
    ...
    return res.status(200).json({ success: true, date: today, news: items.length, source: "sonnet-web-search" });
